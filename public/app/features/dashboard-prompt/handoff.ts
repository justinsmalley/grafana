...
interface StartPlanningArgs {
  ...
  /** Folders the user attached as context on the landing prompt. */
  folders?: PromptDashboardRef[];
...
function buildAttachedResourceContext(args: StartPlanningArgs): ChatContextItem[] {
  ...
  for (const dashboard of args.dashboards ?? []) {
    items.push(
  ...
  for (const folder of args.folders ?? []) {
    items.push(
      createAssistantContextItem('dashboard_folder', {
...
export function buildPlanningInstructions(args: StartPlanningArgs): string {
  ...
  if (args.folders && args.folders.length > 0) {
    parts.push(`Folders the user attached as context:\n${formatDashboardRefs(args.folders)}`);
  }
